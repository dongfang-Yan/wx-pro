...
function getSeriesDataItem(series, index) {
    ...
    series.forEach(function (item) {
        if (item.data[index] !== null && typeof item.data[index] !== 'undefined') {
            ...
            seriesItem.data = item.format ? item.format(item.data[index]).toFixed(2) : item.data[index].toFixed(2);
...
function getPieTextMaxLength(series) {
    ...
    var maxLength = 0;
    series.forEach(function (item) {
        var text = item.format ? item.format(+item._proportion_.toFixed(2)) : util.toFixed(item._proportion_ * 100).toFixed(2) + '%';
...
function drawPieText(series, opts, config, context, radius, center) {
    ...
    var seriesConvert = series.map(function (item) {
        ...
        var text = item.format ? item.format(+item._proportion_) : util.toFixed(item._proportion_ * 100).toFixed(2) + '%';
